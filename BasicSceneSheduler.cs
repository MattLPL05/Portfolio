...
public class BasicSceneSheduler : MonoBehaviour
{
    ...
    [SerializeField]
    private float Delay; //delay, in seconds
    ...
    //If scene is not specific, text is not required
     ...
     void OnTriggerEnter(Collider other)
     {
        ...
        if(IsPsychicial == true)
        {
            if(IsSpecific == true)
            {
            StartCoroutine(SheduleSceneSwitch());
            }
    ...
    private IEnumerator SheduleSceneSwitch()
    {
